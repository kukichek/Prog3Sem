...
Set::Set(std::initializer_list<int> l)
	: size_s(0), capacity(max(1, size_s)) {
		ptr = new int [capacity];
...
void Set::accept(Visitor &visitor) {
	...
	while (!it->isDone()) {
		visitor.visitSetEl(it->currentItem());
		it->next();
	}
}
...
void GetModel::visitSetEl(int item) {
	if (stringModel.size() != 0) {
		stringModel += "\r\n";
	}

	stringModel += std::to_string(item);
}

std::string GetModel::getModelString() {
	return stringModel;
}
